...
import GooglePurchaseTracker from "./GooglePurchaseTracker";

type SuccessPageProps = {
  searchParams?: Promise<{
    session_id?: string;
    test_purchase?: string;
  }>;
};
...
export default async function SuccessPage({
  ...
  const isTestPurchase = resolvedSearchParams?.test_purchase === "1";
  const shouldTrackPurchase = hasSession || isTestPurchase;

  const transactionId = sessionId || `test-purchase-${Date.now()}`;
  const purchaseValue = 19.99;
    ...
    <>
      {shouldTrackPurchase ? (
        <GooglePurchaseTracker
          ...
          value={purchaseValue}
          currency="GBP"
          isTest={isTestPurchase && !hasSession}
        />
      ) : null}
